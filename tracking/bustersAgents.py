        ...
        for belief in livingGhostPositionDistributions:
            ...
            for pos in belief.keys():
                if belief[pos] > maxprob:
                    maxprob = belief[pos]
                    maxpos = pos
            if nearpos == None or self.distancer.getDistance(pacmanPosition, maxpos) < self.distancer.getDistance(pacmanPosition, nearpos):
                nearpos = maxpos
        bestaction=legal[0]
        minidistance=self.distancer.getDistance(pacmanPosition, nearpos)
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            if nearpos == None or self.distancer.getDistance(successorPosition, nearpos) < minidistance:
                minidistance = self.distancer.getDistance(successorPosition, nearpos)
                bestaction = action
        return bestaction
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
