...
def constructBayesNet(gameState: hunters.GameState):
    ...
    maxlen=X_RANGE+Y_RANGE-2+MAX_NOISE+1
    variableDomainsDict[OBS0] = [i for i in range(maxlen)]
    variableDomainsDict[OBS1] = [i for i in range(maxlen)]
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        # print(type(self))
        if total_weight==0 or total_weight==None:
            return None
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldpos in self.allPositions:
            if self.beliefs[oldpos]==0:
                continue
            mut=self.getPositionDistribution(gameState, oldpos)
            for pos in self.allPositions:
                newbeliefs[pos]+=self.beliefs[oldpos]*mut[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        # rest=self.numParticles%len(self.legalPositions)
        # for pos in self.legalPositions:
        #     self.particles.append(pos)
        #     rest-=1
        #     if rest==0:
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in dis.keys():
            dis[i]*=self.getObservationProb(observation,PacmanPosition,i,JailPosition)
        if dis.total()==0:
            self.initializeUniformly(gameState)
        else:
            dis.normalize()
            self.particles=[dis.sample() for i in range(self.numParticles)]
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        newbeliefs=DiscreteDistribution()
        dis=self.getBeliefDistribution()
        for oldpos in dis.keys():
            if dis[oldpos]==0:
                continue
            mut=self.getPositionDistribution(gameState, oldpos)
            for pos in dis.keys():
                newbeliefs[pos]+=dis[oldpos]*mut[pos]
        dis=newbeliefs
        if dis.total()==0:
            self.initializeUniformly(gameState)
        else:
