...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # print(findModel(conjoin([premise,~conclusion])))
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    if logic.pl_true(~inverse_statement,assignments):
        return True
    return False
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    return conjoin([LeastOne,MostOne])
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    Rhs=disjoin(possible_causes)
    # conjoin([~PropSymbolExpr(pacman_str, x, y, time=last),~PropSymbolExpr(wall_str, x, y),
    return PropSymbolExpr(pacman_str, x, y, time=now) % Rhs
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    alc=[]
    for i in all_coords:
        alc.append(PropSymbolExpr(wall_str,i[0],i[1])>>(~PropSymbolExpr(pacman_str,i[0],i[1],time=t)))
        # >>(~PropSymbolExpr(pacman_str,i[0],i[1],t))PropSymbolExpr(wall_str,i[0],i[1])
    pacphysics_sentences.append(conjoin(alc))
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(pacman_str,i[0],i[1],time=t) for i in non_outer_wall_coords]))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB1=KB+[PropSymbolExpr(pacman_str,x1,y1,time=1)]
    model1=findModel(conjoin(KB1))
    KB2=KB+[~PropSymbolExpr(pacman_str,x1,y1,time=1)]
    model2=findModel(conjoin(KB2))
...
def positionLogicPlan(problem) -> List:
    ...
    tm=0
    while tm<=50:
        KB.append(exactlyOne([PropSymbolExpr(pacman_str,x,y,time=tm) for x,y in non_wall_coords]))
        model=findModel(conjoin(KB+[PropSymbolExpr(pacman_str,xg,yg,time=tm)]))
        if model!=False:
            return extractActionSequence(model,actions)
        KB.append(exactlyOne([PropSymbolExpr(action,time=tm) for action in actions]))
        for x,y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x,y,tm+1,walls_grid))
        tm+=1
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    while tm<=50:
        ...
        Tmp=KB.copy()
        for x,y in non_wall_coords:
            Tmp.append(~PropSymbolExpr(food_str,x,y,time=tm))
        model=findModel(conjoin(Tmp))
        if model!=False:
            return extractActionSequence(model,actions)
        KB.append(exactlyOne([PropSymbolExpr(action,time=tm) for action in actions]))
        for x,y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x,y,tm+1,walls))
        for x,y in non_wall_coords:
            KB.append(~PropSymbolExpr(food_str,x,y,time=tm+1)%(PropSymbolExpr(pacman_str,x,y,time=tm)|~PropSymbolExpr(food_str,x,y,time=tm)))
        tm+=1
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            ...
            if entails(conjoin(KB),PropSymbolExpr(pacman_str,x,y,time=t)):
                ...
                KB.append(~PropSymbolExpr(pacman_str,x,y,time=t))
            # if entails(conjoin(KB),PropSymbolExpr(pacman_str,x,y,time=t)) and entails(conjoin(KB),~PropSymbolExpr(pacman_str,x,y,time=t)):
            #     print("Error: Contradiction in KB")
        ...
        for x,y in all_coords:
            if known_map[x][y]==-1:
                if entails(conjoin(KB),PropSymbolExpr(wall_str,x,y)):
                    ...
                    KB.append(~PropSymbolExpr(wall_str,x,y))
                    known_map[x][y]=0
                if entails(conjoin(KB),PropSymbolExpr(wall_str,x,y)) and entails(conjoin(KB),~PropSymbolExpr(wall_str,x,y)):
                    print("Error: Contradiction in KB")
        agent.moveToNextState(agent.actions[t])
        yield known_map
    # util.raiseNotDefined()
